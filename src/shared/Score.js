...
const Message = ({ user, score, errorMessage, awaitingResponse }) => {
  return (
    <React.Fragment>
      ...
      {awaitingResponse ? (
        <LoadingSpinner
          style={{ width: 30, height: 30, margin: '10px auto' }}
        />
      ) : user ? (
        ...
        <p className="score__message">
          {/* {errorMessage || 'Sign up to save your streak'} */}
          Score service is currently not available
        </p>
      )}
    </React.Fragment>
  );
...
class Score extends Component {
  ...
  handleSubmit = e => {
    ...
    if (isSigningUp) {
      return signup(emailInput, passwordInput, score).then(this.handleLoggedin);
    }
    return signin(emailInput, passwordInput).then(this.handleLoggedin);
  };
  handleLogout = () => {
            ...
            <div>
              {formVisible ? null : (
                <React.Fragment>
                  <button
                    disabled
                    onClick={() => this.showForm({ isSigningUp: true })}
                  >
                    Sign up
                  </button>
                  <button
                    disabled
                    onClick={() => this.showForm({ isSigningUp: false })}
                  >
                    Sign in
                  </button>
                </React.Fragment>
